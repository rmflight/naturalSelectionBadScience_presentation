...
* [**The natural selection of bad science**](http://rsos.royalsocietypublishing.org/content/3/9/160384)
    * Paul E. Smaldino, Richard McElreath, RSOS, 21 September 2016 DOI: 10.1098/rsos.160384
* [**The Inevitable Evolution of Bad Science**](http://www.theatlantic.com/science/archive/2016/09/the-inevitable-evolution-of-bad-science/500609/)
    * Ed Yong, The Atlantic, 21 September 2016
...
cut_plot <- ggplot(cut_data, aes(x = try, y = p.value)) + geom_line(color = "red") + coord_cartesian(xlim = x_range, ylim = y_range)
...
for (isim in 1:50) {
  ...
  c_plot <- cut_plot + geom_point(data = sig_data, size = 3) + coord_cartesian(xlim = x_range, ylim = y_range)
...
pub_plot <- cut_plot + geom_point(data = sig_data, size = 3) + geom_point(data = published_data, size = 3, color = "green") + coord_cartesian(xlim = x_range, ylim = y_range)
...
grp_diff_0 <- data_w_offset(grp_1, grp_2)
...
## Has Power of Studies Improved Over Time?
...
* Effort and power are independent
</div>
...
* Three phases: hypothesis selection, investigation and communication
* Novel hypothesis or replication
* When true hypotheses are rarer, false discoveries will occur more frequently
* Increases in effort do not also increase power
* Makes sense when power is maintained by contingent procedures that are invoked conditional on the nature of the evidence

## Model: Publication

* Assume that positive novel results are always publishable, while negative novel results never are
* Positive and negative replications are communicated with probabilities cR+ and cR−, respectively
* Communicated results enter the published literature
    * Payoffs for communication
    * Different for novel *vs* replicated
    
## Model: Selection & Replication

* Oldest lab dies
* From random sample of labs, highest payoff lab replicates
    * Number of publications is measure of fitness
* Favor those strategies that increase payoff most quickly
...
<figure>
<img src = "other_figs/F5.large.jpg">
...
<figure>
<img src = "other_figs/F6.large.jpg">
